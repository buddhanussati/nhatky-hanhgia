const BADGES = [
    ...
    { id: 'log_50', icon: 'fab fa-envira', color: '#fdcb6e', title: 'Thiện Pháp', desc: 'Hoàn thành 50 thời thiền', condition: (data) => data.logs.length >= 50 },
    { id: 'log_100', icon: 'fas fa-trees', color: '#badc58', title: 'Thiện Nghiệp', desc: 'Hoàn thành 100 thời thiền', condition: (data) => data.logs.length >= 100 },
    { id: 'log_150', icon: 'far fa-cloud-sun', color: '#ff7675', title: 'Ly Cấu', desc: 'Hoàn thành 150 thời thiền', condition: (data) => data.logs.length >= 150 },
    ...
    { id: 'log_250', icon: 'fas fa-mountains', color: '#48dbfb', title: 'Núi Tuyết', desc: 'Hoàn thành 250 thời thiền', condition: (data) => data.logs.length >= 250 },
    ...
    { id: 'log_350', icon: 'far fa-vihara', color: '#ff9f43', title: 'An Trú', desc: 'Hoàn thành 350 thời thiền', condition: (data) => data.logs.length >= 350 },
    ...
    { id: 'log_800', icon: 'fas fa-dove', color: '#dfe6e9', title: 'Vô Lậu', desc: 'Hoàn thành 800 thời thiền', condition: (data) => data.logs.length >= 800 },
    { id: 'log_1000', icon: 'far fa-dharmachakra', color: '#f9ca24', title: 'Chuyển Pháp Luân', desc: 'Hoàn thành 1000 thời thiền', condition: (data) => data.logs.length >= 1000 },
 ...
 { id: 'time_1h', icon: 'fas fa-hourglass-half', color: '#f1c40f', title: 'Hiện Tại', desc: 'Tích lũy 1 giờ tu tập', condition: (data) => data.logs.reduce((s, l) => s + l.minutes, 0) >= 60 },
	...
	{ id: 'time_100h', icon: 'fas fa-fire-burner', color: '#ee5253', title: 'Nhiệt Thành', desc: 'Tích lũy 100 giờ tu tập', condition: (data) => data.logs.reduce((s, l) => s + l.minutes, 0) >= 6000 },
	    { id: 'time_250h', icon: 'fas fa-fire-flame', color: '#f368e0', title: 'Nhiệt Tâm', desc: 'Tích lũy 150 giờ tu tập', condition: (data) => data.logs.reduce((s, l) => s + l.minutes, 0) >= 9000 },
    { id: 'time_150h', icon: 'far fa-fire', color: '#ff6b6b', title: 'Lửa Tuệ', desc: 'Tích lũy 200 giờ tu tập', condition: (data) => data.logs.reduce((s, l) => s + l.minutes, 0) >= 12000 },
    ...
    { id: 'time_300h', icon: 'fas fa-bolt-lightning', color: '#feca57', title: 'Tuệ Căn', desc: 'Tích lũy 300 giờ tu tập', condition: (data) => data.logs.reduce((s, l) => s + l.minutes, 0) >= 18000 },
    { id: 'time_400h', icon: 'fas fa-bullseye-arrow', color: '#ff9ff3', title: 'Tuệ Lực', desc: 'Tích lũy 400 giờ tu tập', condition: (data) => data.logs.reduce((s, l) => s + l.minutes, 0) >= 24000 },
    ...
    { id: 'sit_20', icon: 'fas fa-hand-holding-hand', color: '#cd84f1', title: 'Từ Bi', desc: 'Thời thiền ≥ 20 phút', condition: (data) => data.logs.some(l => l.minutes >= 20) },
    ...
    { id: 'sit_45', icon: 'fab fa-deskpro', color: '#48dbfb', title: 'Voi Chúa', desc: 'Thời thiền ≥ 45 phút', condition: (data) => data.logs.some(l => l.minutes >= 45) },
    ...
    { id: 'sit_180', icon: 'far fa-sailboat', color: '#74b9ff', title: 'Buông Xả', desc: 'Thời thiền ≥ 3 giờ', condition: (data) => data.logs.some(l => l.minutes >= 180) },
...
{ id: 'note_long', icon: 'fa-raindrops', color: '#74b9ff', title: 'Không Phóng Dật', desc: 'Viết ghi chú dài hơn 50 ký tự', condition: (data) => data.logs.some(l => l.notes && l.notes.length > 50) },
    ...
    { id: 'goal-early_5am', icon: 'fa-diamonds-4', color: '#7ed6df', title: 'Tứ Diệu Đế', desc: 'Hoàn thành thời thiền trước 5:00 sáng', condition: (data) => data.logs.some(l => new Date(l.timestamp).getHours() < 5) },
    { id: 'goal-early_3d', icon: 'fas fa-stars', color: '#ffbe76', title: 'Ước Nguyện', desc: '3 ngày liên tiếp thiền trước 5:00 sáng', condition: (data) => { const earlyLogs = data.logs.filter(l => new Date(l.timestamp).getHours() < 5).map(l => l.date); const uniqueDays = [...new Set(earlyLogs)].sort(); let streak = 0; for(let i=0; i<uniqueDays.length-1; i++) { const d1 = new Date(uniqueDays[i]); const d2 = new Date(uniqueDays[i+1]); if ((d2-d1) <= 86400000) streak++; else streak = 0; if(streak >= 2) return true; } return false; }},
	{ id: 'goal-early_7d', icon: 'fa-ribbon', color: '#ff9f43', title: 'Cứu Cánh', desc: '7 ngày liên tiếp thiền trước 5:00 sáng', condition: (data) => { const earlyLogs = data.logs.filter(l => new Date(l.timestamp).getHours() < 5).map(l => l.date); const uniqueDays = [...new Set(earlyLogs)].sort(); let streak = 0; for(let i=0; i<uniqueDays.length-1; i++) { const d1 = new Date(uniqueDays[i]); const d2 = new Date(uniqueDays[i+1]); if ((d2-d1) <= 86400000) streak++; else streak = 0; if(streak >= 6) return true; } return false; }},
    ...
    { id: 'daily_volume_2h', icon: 'fab fa-battle-net', color: '#6a89cc', title: 'Như lý Tác Ý', desc: 'Tổng thời gian thiền ≥ 2 giờ trong 1 ngày', condition: (data) => { const daily = {}; data.logs.forEach(l => daily[l.date] = (daily[l.date]||0) + l.minutes); return Object.values(daily).some(m => m >= 120); }},
    ...
    { id: 'daily_volume_8h', icon: 'far fa-crystal-ball', color: '#a29bfe', title: 'Vượt Thời Gian', desc: 'Tổng thời gian thiền ≥ 8 giờ trong 1 ngày', condition: (data) => { const daily = {}; data.logs.forEach(l => daily[l.date] = (daily[l.date]||0) + l.minutes); return Object.values(daily).some(m => m >= 480); }},
    ...
    { id: 'dedicated_day', icon: 'far fa-sparkles', color: '#badc58', title: 'Tam Bảo', desc: 'Thực hiện 3 thời thiền riêng biệt tổng ≥ 60p trong ngày', condition: (data) => { const goodLogs = data.logs.filter(l => l.minutes >= 60); const counts = {}; goodLogs.forEach(l => counts[l.date] = (counts[l.date]||0)+1); return Object.values(counts).some(c => c >= 3); }},
    ...
    { id: 'long_haul', icon: 'far fa-wreath-laurel', color: '#badc58', title: 'Kiên Trì', desc: 'Hoàn thành 10 phiên dài (≥ 60p)', condition: (data) => data.logs.filter(l => l.minutes >= 60).length >= 10 },
